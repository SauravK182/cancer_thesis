...
# Author: Saurav Kiri
# Date: 2022-11-23
# Description: Trims gzipped reads in a passed directory for adapter sequences and quality
...
# Usage: bash ~/project/scripts/preprocessing/03a_trimming_gz.sh <fastq-dir> <out-dir> <q-score> <minlen> <reports-dir> <se/pe>
#########################
...
if [ ${FLAG} == "se" ]
then
    PREFIX_LIST=$(ls *fastq.gz | sed -r "s/[.]fastq[.]gz//")
    ...
    do
        ...
        bbduk.sh in="${prefix}.fastq.gz" out="${TRIMOUT_DIR}${prefix}_trimmed.fastq.gz" \
        ktrim=r \
        k=23 \
        mink=11 \
        hdist=1 \
    ...
    PREFIX_LIST=$(ls *fastq.gz | sed -r "s/_[12][.]fastq[.]gz//" | uniq)
    echo -e "The FASTQ files to be trimmed are:\n${PREFIX_LIST}"
    ...
    do
        ...
        bbduk.sh in1="${prefix}_1.fastq.gz" in2="${prefix}_2.fastq.gz" \
        out1="${TRIMOUT_DIR}${prefix}_1_trimmed.fastq.gz" out2="${TRIMOUT_DIR}${prefix}_2_trimmed.fastq.gz" \
        ktrim=r \
        k=23 \
        mink=11 \
        hdist=1 \
...
fastqc --threads 6 --outdir ${QC_DIR} ${TRIMOUT_DIR}*.fastq.gz
